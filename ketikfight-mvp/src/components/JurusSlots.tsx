        ...
        {slots.map((slot, i) => {
          ...
          return (
            <div
              key={i}
              className={`
                w-28 h-14 rounded-lg border-2 px-2 py-1 flex flex-col items-center justify-center
                transition-all ${tierBg[j.tier]}
                ${matched ? "scale-105 " + tierColors[j.tier] : "border-gray-700"}
                ${isDefense ? "ring-1 ring-cyan-500/50" : ""}
              `}
              style={matched ? { borderColor: character.color, boxShadow: `0 0 8px ${character.color}40` } : {}}
            >
